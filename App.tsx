...
const App: React.FC = () => {
    ...
    const [currentUser, setCurrentUser] = useState<{email: string, role: string, name: string, picture: string} | null>(null);
    ...
    useEffect(() => {
        const email = sessionStorage.getItem('loggedInUserEmail') || ARCHITECT_EMAIL;
        const role = sessionStorage.getItem('userRole') || 'architect';
        const name = sessionStorage.getItem('userName') || 'Dr. Architect';
        const picture = sessionStorage.getItem('userPicture') || '';
        setCurrentUser({ email, role, name, picture });
    }, []);

    const addNotification = useCallback((title: TranslationKey, message: TranslationKey, type: 'success' | 'error' | 'info', data?: string) => {
        const newNotification: Notification = { id: Date.now(), title, message, type, data };
    ...
    useEffect(() => {
        ...
        const handleProtocolExecution = (envelope: any) => {
            ...
            if (task && !isRefactoring) {
                ...
                setTimeout(() => {
                    ...
                    aetherBus.publish('REFACTORING_COMPLETE', { completedTaskId: task.id, newFiles: newFiles });
                }, 1500);
            }
        };
        const handleRefactoringComplete = (envelope: any) => {
            ...
            if (completedTaskId === 'CIRCULAR_DEP_AUTH_CLIENT') fileToSelect = newFiles.find(f => f.path === 'src/services/tokenProvider.ts');
            else if (completedTaskId.startsWith('SPLIT_UTILITIES')) fileToSelect = newFiles.find(f => f.path === 'src/utils/formatters-date.ts');
        ...
        const handleSimulateImpact = (envelope: any) => {
            ...
            const affected = analysisResult.dependencies.filter(dep => filesInvolved.includes(dep.to)).map(dep => dep.from);
    ...
    const handleOpenGitHubModal = () => {
        const completedTaskTitles = completedTasks.map(id => allTasks.find(task => task.id === id)).filter(Boolean).map(task => t(task!.titleKey));
        const description = completedTaskTitles.length > 0 ? `feat: AI-assisted refactoring\n\nThis pull request includes the following automated refactors:\n- ${completedTaskTitles.join('\n- ')}` : 'chore: Aetherium Genesis code sync';
    ...
    const handleCreatePullRequest = () => {
        ...
        setTimeout(() => { setGithubModalStep('success'); setIsGitHubModalOpen(true); }, 500);
        setTimeout(() => { setGithubModalStep('success'); }, 2000);
    };
    
    const handleShowHub = () => setActiveView('hub');

    const handleLaunchModule = (tab: AppTab | 'aether-canvas' | 'settings') => {
        if (tab === 'aether' && !isGenesisModeActive) { // Special case for Core Monitor vs Aether Canvas
             setActiveTab('aether');
        } else if (tab === 'aether-canvas') {
            setIsGenesisModeActive(true);
            setActiveTab('aether');
        } else if (tab === 'settings') {
            setIsModelConfigModalOpen(true);
            return; // Don't switch to IDE view
        } else {
    ...
    const graphData = useMemo(() => {
        ...
        const nodes = files.map(f => ({ id: f.path, isDead: analysisResult.deadCodeFiles.includes(f.path) ?? false, isCircular: analysisResult.circularDependencyFiles.flat().includes(f.path) ?? false, heat: analysisResult.heatScores.get(f.path) ?? 0, }));
        const links = analysisResult.dependencies.map(d => ({ source: d.from, target: d.to, }));
        return { nodes, links };
    ...
    const TAB_TITLE_KEYS: Record<AppTab, TranslationKey> = {
        genesis: 'imageGenesis', analysis: 'imageAnalysis', chat: 'chatbot', fabric: 'dataFabric',
        economicFabric: 'economicFabric', aether: 'aetherInterface', agent: 'aiAgent',
    ...
    if (!currentUser) {
        return <div className="flex h-screen w-full items-center justify-center bg-gray-900"><SpinnerIcon className="w-10 h-10 animate-spin text-cyan-400" /></div>;
    }
        ...
        <div className="flex flex-col h-screen bg-gray-900 text-gray-200 font-sans">
            ...
            <main className="flex flex-1 overflow-hidden">
                <div className="bg-gray-800 border-r border-gray-700 flex flex-col flex-shrink-0 w-1/4 lg:w-1/5">
                   <FileExplorer files={files} selectedFile={selectedFile} onSelectFile={handleSelectFile} analysisResult={analysisResult} />
                </div>
                ...
                <div className={`bg-gray-800 border-l border-gray-700 flex flex-col transition-all duration-300 ease-in-out overflow-hidden ${focusedView === 'panel' ? 'flex-1' : 'w-[30%] 2xl:w-1/4 flex-shrink-0'}`}>
                    {focusedView === 'panel' && (
                        <div className="p-2 border-b border-gray-700 flex items-center flex-shrink-0">
                            <button onClick={() => { pushToHistory(); setFocusedView('editor'); }} className="flex items-center p-2 text-sm rounded-md text-gray-300 hover:bg-gray-700 transition-colors" title={t('backToEditor')}>
                                <CodeIcon className="w-5 h-5 mr-2" /> {t('backToEditor')}
                            </button>
                    ...
                    {focusedView === 'editor' && (
                         <div className="grid grid-cols-5 lg:grid-cols-10 border-b border-gray-700 flex-shrink-0">
                            ...
                            <button onClick={() => handleFocusPanel('cli')} className={`p-3 text-sm font-semibold transition-colors ${activeTab === 'cli' ? 'bg-gray-900 text-cyan-400' : 'text-gray-400 hover:bg-gray-700'}`}>{t('cliExtensionHub')}</button>
                             <button onClick={() => handleFocusPanel('assurance')} className={`p-3 text-sm font-semibold transition-colors ${activeTab === 'assurance' ? 'bg-gray-900 text-cyan-400' : 'text-gray-400 hover:bg-gray-700'}`}>{t('systemAssurance')}</button>
                    ...
                    <div className="flex-grow relative overflow-y-auto">
                       ...
                       {activeTab === 'economicFabric' && <EconomicFabricView addNotification={addNotification} userEmail={currentUser.email} />}
                       {activeTab === 'cli' && <CliExtensionView />}
                       {activeTab === 'assurance' && <SystemAssuranceView />}
                       {activeTab === 'aether' && (
                           <div className="flex flex-col h-full text-sm">
                               {isGenesisModeActive ? ( <AetherCanvas onExit={() => setIsGenesisModeActive(false)} /> ) : (
                                   <>
                                   <div className="flex-grow relative">
                                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none"><p className="text-gray-700 font-mono text-xs">{t('coreMonitorStatus')}</p></div>
                                    ...
                                    <div className="p-2 bg-gray-900/50 border-t border-gray-700">
                                        ...
                                        <div className="mt-2 p-2 border-t border-gray-700/50">
                                            <button onClick={() => { setIsGenesisModeActive(true); setActiveView('ide'); }} className="w-full mb-2 flex items-center justify-center px-3 py-2 bg-purple-600 hover:bg-purple-500 text-white font-semibold rounded-md transition-colors">
                                                <SparklesIcon className="w-4 h-4 mr-2" /> {t('activateGenesisProtocol')}
                                            </button>
                                            ...
                                            {isDevMode && (
                                                <div className="mt-2 space-y-2 text-xs">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 items-center">
                                                         <label className="text-gray-400">{t('devParamFrequency')}</label><input type="range" min="0.1" max="20" step="0.1" value={devParams.frequency} onChange={e => setDevParams(p => ({...p, frequency: +e.target.value}))} className="w-full" />
                                                         <label className="text-gray-400">{t('devParamIntensity')}</label><input type="range" min="0.1" max="3" step="0.1" value={devParams.intensity} onChange={e => setDevParams(p => ({...p, intensity: +e.target.value}))} className="w-full" />
                                                         <label className="text-gray-400">{t('devParamDecay')}</label><input type="range" min="0.8" max="0.99" step="0.01" value={devParams.decay} onChange={e => setDevParams(p => ({...p, decay: +e.target.value}))} className="w-full" />
                                                         <label className="text-gray-400">{t('devParamChaos')}</label><input type="range" min="0" max="10" step="0.1" value={devParams.chaos} onChange={e => setDevParams(p => ({...p, chaos: +e.target.value}))} className="w-full" />
                                                         <label className="text-gray-400">{t('devParamColor')}</label><input type="color" value={devParams.color} onChange={e => setDevParams(p => ({...p, color: e.target.value}))} className="w-full bg-gray-700" />
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                        ...
                        {activeTab === 'agent' && <AgentPanel tasks={allTasks} isRefactoring={isRefactoring} completedTasks={completedTasks} />}
                        {activeTab === 'graph' && <DependencyGraph nodes={graphData.nodes} links={graphData.links} highlightedNode={highlightedNode} impactAnalysis={impactAnalysis} />}
                    </div>
                </div>
            </main>

            <div className="absolute top-20 right-0 z-50">
                {notifications.map(notification => <NotificationComponent key={notification.id} notification={notification} onDismiss={removeNotification} />)}
            </div>

            {isGitHubModalOpen && (
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" role="dialog" aria-modal="true" aria-labelledby="github-modal-title">
                    <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-xl w-full max-w-2xl">
                        <div className="p-4 border-b border-gray-700 flex justify-between items-center">
                            <h3 id="github-modal-title" className="font-semibold text-lg text-gray-200 flex items-center"><GitHubIcon className="w-6 h-6 mr-3 text-cyan-400" />{t('githubModalTitle')}</h3>
                            <button onClick={() => setIsGitHubModalOpen(false)} className="p-2 text-gray-400 rounded-full hover:bg-gray-700 hover:text-white transition-colors" aria-label="Close"><CloseIcon className="w-5 h-5" /></button>
                        </div>
                        <div className="p-6">
                            {githubModalStep === 'form' && (
                                <form onSubmit={(e) => { e.preventDefault(); setIsPrConfirmModalOpen(true); }} className="space-y-6">
                                    <div><label htmlFor="repo-name" className="block text-sm font-medium text-gray-400 mb-1">{t('repositoryName')}</label><input type="text" id="repo-name" value={repoName} onChange={(e) => setRepoName(e.target.value)} className="w-full p-2 bg-gray-900 border border-gray-600 rounded-md focus:ring-2 focus:ring-cyan-500 focus:outline-none text-gray-200" /></div>
                                    <div className="border-t border-gray-700 pt-6">
                                        <h4 className="text-md font-semibold text-gray-300 mb-3">{t('commitDetails')}</h4>
                                        <div><label htmlFor="commit-message" className="block text-sm font-medium text-gray-400 mb-1">{t('commitMessage')}</label><input id="commit-message" value={commitMessage} onChange={(e) => setCommitMessage(e.target.value)} className="w-full p-2 bg-gray-900 border border-gray-600 rounded-md focus:ring-2 focus:ring-cyan-500 focus:outline-none text-gray-200" /></div>
                                    </div>
                                    <div className="border-t border-gray-700 pt-6">
                                        <h4 className="text-md font-semibold text-gray-300 mb-3">{t('pullRequestDetails')}</h4>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                            <div><label htmlFor="base-branch" className="block text-sm font-medium text-gray-400 mb-1 flex items-center"><GitBranchIcon className="w-4 h-4 mr-2" /> {t('baseBranch')}</label><input id="base-branch" value={baseBranch} onChange={(e) => setBaseBranch(e.target.value)} className="w-full p-2 bg-gray-900 border border-gray-600 rounded-md focus:ring-2 focus:ring-cyan-500 focus:outline-none text-gray-200" /></div>
                                            <div><label htmlFor="compare-branch" className="block text-sm font-medium text-gray-400 mb-1 flex items-center"><GitBranchIcon className="w-4 h-4 mr-2" /> {t('compareBranch')}</label><input id="compare-branch" value={compareBranch} onChange={(e) => setCompareBranch(e.target.value)} className="w-full p-2 bg-gray-900 border border-gray-600 rounded-md focus:ring-2 focus:ring-cyan-500 focus:outline-none text-gray-200" /></div>
                                        </div>
                                        <div><label htmlFor="pr-title" className="block text-sm font-medium text-gray-400 mb-1">{t('pullRequestTitle')}</label><input id="pr-title" value={prTitle} onChange={(e) => setPrTitle(e.target.value)} className="w-full p-2 bg-gray-900 border border-gray-600 rounded-md focus:ring-2 focus:ring-cyan-500 focus:outline-none text-gray-200" /></div>
                                        <div className="mt-4"><label htmlFor="pr-description" className="block text-sm font-medium text-gray-400 mb-1">{t('pullRequestDescription')}</label><textarea id="pr-description" value={prDescription} onChange={(e) => setPrDescription(e.target.value)} rows={6} className="w-full p-2 bg-gray-900 border border-gray-600 rounded-md focus:ring-2 focus:ring-cyan-500 focus:outline-none text-gray-200 resize-none" /></div>
                                    </div>
                                    <div className="flex justify-end pt-2"><button type="submit" className="flex items-center justify-center px-4 py-2 bg-green-600 hover:bg-green-500 text-white font-bold rounded-md transition-colors"><GitHubIcon className="w-5 h-5 mr-2" />{t('createPullRequest')}</button></div>
                                </form>
                            )}
                            {githubModalStep === 'loading' && <div className="flex flex-col items-center justify-center h-48 text-gray-400"><SpinnerIcon className="w-10 h-10 animate-spin text-cyan-400" /><p className="mt-4 text-lg">{t('creatingPullRequest')}</p></div>}
                            {githubModalStep === 'success' && <div className="flex flex-col items-center justify-center h-48 text-center"><CheckCircleIcon className="w-16 h-16 text-green-400" /><h4 className="mt-4 text-xl font-bold text-gray-100">{t('pullRequestCreated')}</h4><a href={`https://github.com/${repoName}/pull/1`} target="_blank" rel="noopener noreferrer" className="mt-2 text-cyan-400 hover:underline">{t('viewOnGitHub')}</a><button onClick={() => setIsGitHubModalOpen(false)} className="mt-6 px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white font-bold rounded-md transition-colors">{t('close')}</button></div>}
                        </div>
                    </div>
                </div>
            )}
            <ConfirmationModal isOpen={isPrConfirmModalOpen} onClose={() => setIsPrConfirmModalOpen(false)} onConfirm={() => { setIsPrConfirmModalOpen(false); handleCreatePullRequest(); }}
                title={t('confirmPrTitle')} message={t('confirmPrMessage')} confirmText={t('confirm')} cancelText={t('cancel')}
                icon={<GitHubIcon className="w-6 h-6 mr-3 text-cyan-400" />} confirmButtonClass="bg-green-600 hover:bg-green-500" />
            <ModelConfigModal isOpen={isModelConfigModalOpen} onClose={() => setIsModelConfigModalOpen(false)} currentConfig={modelConfig}
                onSave={(newConfig) => { setModelConfig(newConfig); setIsModelConfigModalOpen(false); }} />
            <UserProfilePanel isOpen={isUserProfileOpen} onClose={() => setIsUserProfileOpen(false)} userEmail={currentUser.email}
                filesManaged={files.length} protocolsExecuted={completedTasks.length} imagesSynthesized={12} analysesPerformed={4} />
        </div>
    );
};
